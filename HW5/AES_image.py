...
def ctr_aes_image(iv,image,enc_image, key):
    #open ppm file and read header
    Fimage = open(image, 'rb')
    image = Fimage.readlines()[3:]
    Fimage.seek(0)   
    header = Fimage.readlines()[0:3] 
    ...
    image_bv = BitVector(size=0)

    #convert the header to bitvector and combine the header correctly
    for i in range(3):
        header_temp = BitVector(hexstring = header[i].hex())
        header_bv += header_temp

    #convert the image file to a bit vector
    for line in image:
        image_temp = BitVector(hexstring = line.hex())
        image_bv += image_temp

    full_size = len(image_bv)

    #append header to the out file
    FILEhead = open(enc_image, 'ab')
    header_bv.write_to_file(FILEhead)
    ...
    while x < full_size:
        ...
        for i in range(15):
            #if sys.argv[1] == str('-e'):
            if i == 0:
                post_round = round_keys_first(iv, round_keys)
            elif i == 1:
                ...
                final = curr_block
                if i == 14:   
                    #XOR and write to out file
                    test = final ^ curr_bitvec  
                    test.write_to_file(FILEhead)
