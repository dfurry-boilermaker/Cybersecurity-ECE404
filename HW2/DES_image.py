...
def encrypt(bitV_plain, round_key, iteration):
    ...
    while x < full_size:
        ...
        x = x + 64
    return cipher_bv #return the fully encrypted bitvector
...
if __name__ == '__main__':
    ...
    #import image file, header and file seperately
    Fimage = open(sys.argv[1], "rb")
    encryptT = Fimage.readlines()[3:]
    Fimage.seek(0)
    ...
    for line in encryptT:
        image_temp = BitVector(hexstring = line.hex())
        image_bv += image_temp

    #call encrypt function
    for j in range(16):
            encryptPPM = encrypt(image_bv, round_keys[j], j)
            print(j)
    
    encryptFinal = header_bv + encryptPPM #combine the bitvector header and the encrypted file

    #write to image_enc.ppm 
    FILEfinal = open(sys.argv[3], 'wb')
    encryptFinal.write_to_file(FILEfinal)